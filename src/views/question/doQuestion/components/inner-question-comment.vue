...
const props = defineProps({
  comments: {
    type: Array as PropType<QuestionCommentVO[]>,
    default: () => [],
  },
});
...
// 本地维护的评论列表，用于处理删除等操作
const localComments = ref<QuestionCommentVO[]>([]);
...
const commentChildrenMap = reactive<{ [key: string]: QuestionCommentVO[] }>({});
...
// 判断评论是否被展开
const isCommentExpanded = (comment: QuestionCommentVO) => {
  return comment.id ? expandedCommentsMap[comment.id] : false;
};

// 判断评论是否是当前登录用户发表的
const isCurrentUserComment = (comment: QuestionCommentVO) => {
 ...
 * 还需要更新本地的评论数据结构
 */
const handleConfirmDelete = async (comment: QuestionCommentVO) => {
  if (!comment || !comment.id) return;

  try {
    const res = await QuestionCommentControllerService.removeUsingDelete1(
      comment.id + ""
    );
...
const checkParentCommentStatus = async (parentId: number) => {
  ...
  try {
    ...
    // 找到合适的questionId
    let questionId = null;
    if (props.comments.length > 0 && props.comments[0].questionId) {
      questionId = props.comments[0].questionId;
    } else if (recoverComment.value.questionId) {
      questionId = recoverComment.value.questionId;
    }
...
// 关闭回复弹出框
const closeReplyPopover = (comment: QuestionCommentVO) => {
  if (comment.id) {
    replyVisibleMap[comment.id] = false;
  }
};
...
// 点击展开，加载子评论
const expandComment = async (comment: QuestionCommentVO) => {
  if (comment.id && comment.questionId) {
    try {
      // 先加载子评论数据
      await loadChildComments(comment.id + "", comment.questionId + "");
...
// 折叠子评论
const collapseComment = (comment: QuestionCommentVO) => {
  if (comment.id) {
    expandedCommentsMap[comment.id] = false;
  }
};
...
// 加载子评论
const loadChildComments = async (parentId: string, questionId: string) => {
  if (!parentId || !questionId) return;
...
const recoverComment = ref<QuestionCommentRequest>({
  content: "",
  questionId: 0,
  parentId: 0,
});

const handleRecoverComment = (comment: QuestionCommentVO) => {
  recoverComment.value.parentId = comment.id;
  recoverComment.value.questionId = comment.questionId;
  // 显示回复弹出框
  if (comment.id) {
 ...
 * 提交回复
 */
const doRecoverComment = async (comment: QuestionCommentVO) => {
